...
# exe化(コンソール)時、既定の文字コードがcp932になり絵文字の出力で
# UnicodeEncodeErrorが発生するため、明示的にUTF-8へ切り替える
...
def find_project_root(start_dir, marker="DMPS", max_up=4):
    """ start_dirから上の階層をたどり、マーカーフォルダ(DMPS)を持つ場所をルートとする。
    onefile/onedirでexeの置かれる深さが変わっても正しくルートを見つけられる。 """
    d = start_dir
...
if getattr(sys, "frozen", False):
    ...
    PROJECT_ROOT = find_project_root(os.path.dirname(os.path.abspath(sys.executable)))
else:
    PROJECT_ROOT = find_project_root(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
def match_and_draw(frame, templates, threshold=0.75):
    ...
    for name, data in templates.items():
        ...
        # キャプチャ画面よりテンプレートが大きい場合はスキップ（サイズ不一致でクラッシュするため）
...
def start_capture():
    ...
    # 同名のウィンドウが複数見つかることがあるため、面積が最大のものを実際のゲーム画面とみなす
    target_window = max(windows, key=lambda w: w.width * w.height)
    ...
    with mss.MSS() as sct:
        while True:
            ...
            # 待機時間なしで全力ループしているとCPU負荷が高くなるため、
            # ここで待つことでキャプチャ頻度を落とす（先攻/後攻・勝敗の検出には十分な頻度）
